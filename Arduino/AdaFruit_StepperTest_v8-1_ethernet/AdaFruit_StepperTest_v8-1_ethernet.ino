...
unsigned long LoopTime; //for loop timer in slewAz loop
unsigned long AbsTime; //for loop timer in slewAs loop
    ...
    if (whichMotor == "a") {
      ...
      if (steps == 0) {                       // if nonnumeric input
        ...
        if (steps == -9999) {                 // -9999 is the command to home
          ...
          AbsTime = millis(); //start a timer before entering while loop
          while ( (DistanceToGo = stepperaz.distanceToGo()) != 0) {
            ...
            LoopTime = millis(); //gets time in loop
            //Serial.print(LoopTime-AbsTime); //for debugging
            //Serial.print('\t'); //for debugging
            //Serial.println(AbsTime); //for debugging
            //if (LoopTime-AbsTime < 0){ //checks to see if millis() has looped during the time of AbsTime and LoopTime
            // AbsTime = LoopTime; //reset AbsTime if it loops to zero during while loop
            //} //need to check that millis() loops to zero and not -mag
            if (LoopTime-AbsTime > 3600000) { //3600000ms = 1hr
              client.stop();
              break; //breaks out of the loop after a given time
            }
          }    // end of while loop
          myStepperAz->release();
        } // end of do steps 
...
void azHome() {
  ...
  while (aztrig){ //rotate until optosensor sees a rising edge.
    ...
    optsen_prev = optsen;
    stepperaz.run();
    //Serial.println(optsen);   //for debugging only *HB change
    //Serial.print(stepperaz.currentPosition());
    //Serial.print("\t");
    //Serial.println(optsen); //for debugging only *HB change end
    ReturnPos();
