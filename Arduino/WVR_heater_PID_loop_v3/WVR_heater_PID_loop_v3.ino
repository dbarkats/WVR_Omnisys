...
int stateRelay = LOW;
const int thres = 3000;  // threshold on  PID Output to enable the additional heater
const int relay =  8;   // pin 8 to control the relay
...
void loop() {
  ...
  // control the AC extra heater bsaed on Output
  if (Output > thres + 20)
    stateRelay = HIGH;
  if (Output < thres - 20)
    stateRelay = LOW;
